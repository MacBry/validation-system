...
def create_html(sn, filename, start_time, num_points=24, interval_mins=15):
...
def main():
    out_dir = "Scenariusz_IDEALNY"
    os.makedirs(out_dir, exist_ok=True)
    
    # Start measurements exactly 24 hours ago
    ...
    for i in range(1, 10):
        ...
        # 4 hours of data, 15 min interval = 16 points
        create_html(sn, filename, start_time=start_time, num_points=16, interval_mins=15)
        
    # 1 reference file
    ref_sn = 50000010
    ref_filename = os.path.join(out_dir, f"REF_{ref_sn}.html")
    create_html(ref_sn, ref_filename, start_time=start_time, num_points=16, interval_mins=15)
